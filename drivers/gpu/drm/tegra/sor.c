...
static int tegra_sor_dp_link_apply_training(struct drm_dp_link *lnk)
{
	...
	for (value = 0, i = 0; i < link->base.lanes; i++) {
		...
		if (link->base.caps.channel_coding)
	...
	if (link->base.caps.tps3_supported)
...
static int tegra_sor_dp_link_configure(struct drm_dp_link *lnk)
{
	struct drm_dp_link_tegra *link = to_drm_dp_link_tegra(lnk);
	struct tegra_sor *sor = container_of(link, struct tegra_sor, link);
	...
	rate = drm_dp_link_rate_to_bw_code(link->base.rate);
	lanes = link->base.lanes;
	...
	if (link->base.caps.enhanced_framing)
