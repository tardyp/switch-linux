...
static int tegra_xudc_powerdomain_init(struct tegra_xudc *xudc)
{
	...
	if (IS_ERR(xudc->genpd_dev_device)) {
		...
		dev_err(dev, "failed to get device pm-domain: %d\n", err);
		return err;
	}

	xudc->genpd_dev_ss = dev_pm_domain_attach_by_name(dev, "xusb_ss");
...
static int tegra_xudc_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(xudc->data_role_extcon)) {
		...
		if (err != -EPROBE_DEFER)
			dev_err(xudc->dev,
				"extcon_get_edev_by_phandle failed %d\n",
				err);
	...
	if (err) {
		dev_err(xudc->dev, "phy init failed %d\n", err);
		goto put_powerdomains;
	}
