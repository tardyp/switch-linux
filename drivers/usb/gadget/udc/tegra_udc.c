...
static int const tegra_udc_extcon_cable[] = {
	EXTCON_USB,
	EXTCON_USB_HOST,
	EXTCON_CHG_USB_SDP,
	EXTCON_CHG_USB_DCP,
...
static void dr_controller_run(struct tegra_udc *udc)
{
	...
	if (can_pullup(udc)) {
		...
		if (udc->charging_supported &&
			(udc->connect_type == EXTCON_CHG_USB_SDP))
...
static void tegra_udc_set_extcon_state(struct tegra_udc *udc)
{
	...
	if (udc->prev_connect_type != udc->connect_type
			&& udc->prev_connect_type != EXTCON_NONE)
		extcon_set_state(edev, udc->prev_connect_type, false);

	if (udc->connect_type != udc->connect_type_lp0
			&& udc->connect_type != EXTCON_NONE)
		extcon_set_state(edev, udc->connect_type, true);
	dev_info(dev, "notification status (%d,%d,%d)\n",
			udc->prev_connect_type, udc->connect_type,
...
static int tegra_usb_set_charging_current(struct tegra_udc *udc)
{
	...
	struct device *dev;
	int ret;
	udc->connect_type = EXTCON_CHG_USB_CDP;
	dev = &udc->pdev->dev;
	...
	case EXTCON_NONE:
		...
		if ((udc->prev_connect_type == EXTCON_CHG_USB_SDP) ||
			(udc->prev_connect_type == EXTCON_CHG_USB_CDP) ||
			(udc->prev_connect_type == EXTCON_CHG_USB_ACA))
	...
	if (!udc->vbus_in_lp0) {
		...
		udc->connect_type_lp0 = EXTCON_NONE;
	}
	return ret;
}
...
static int tegra_detect_cable_type(struct tegra_udc *udc)
{
	...
	if (!udc->charging_supported) {
		tegra_udc_set_charger_type(udc, EXTCON_CHG_USB_SDP);
		return 0;
	}
...
static int tegra_vbus_session(struct usb_gadget *gadget, int is_active)
{
	...
	if (udc->vbus_active && !is_active) {
		...
		tegra_udc_set_charger_type(udc, EXTCON_NONE);
		spin_unlock_irqrestore(&udc->lock, flags);
		...
		if ((udc->connect_type == EXTCON_CHG_USB_SDP) ||
		    (udc->connect_type == EXTCON_CHG_USB_CDP) ||
		    (udc->connect_type == EXTCON_CHG_USB_ACA))
...
static int tegra_vbus_draw(struct usb_gadget *gadget, unsigned mA)
{
	...
	if (udc->connect_type != EXTCON_NONE
		&& udc->connect_type != EXTCON_CHG_USB_SDP
		&& udc->connect_type != EXTCON_CHG_USB_CDP
		&& udc->connect_type != EXTCON_CHG_USB_ACA)
		tegra_udc_set_charger_type(udc, EXTCON_CHG_USB_SDP);
		...
		struct tegra_req *curr_req)
{
	...
	for (j = 0; j < curr_req->dtd_count; j++) {
		...
		if (udc->fence_read)
			readb(udc->ppcs_base + USB1_PREFETCH_ID);
...
static void tegra_udc_non_std_charger_detect_work(struct work_struct *work)
{
	...
	tegra_udc_set_charger_type(udc, EXTCON_CHG_USB_SLOW);
	tegra_usb_set_charging_current(udc);
...
static int tegra_udc_stop(struct usb_gadget *g)
{
	...
	if (udc->driver)
		DBG("%s(%d) unregistered gadget driver '%s'\n",
			 __func__, __LINE__, udc->driver->driver.name);
...
static int __init struct_ep_setup(struct tegra_udc *udc, unsigned char index,
		char *name, int link)
{
	static const struct usb_ep_caps _caps = USB_EP_CAPS(USB_EP_CAPS_TYPE_ALL, USB_EP_CAPS_DIR_ALL);	
...
static int __init tegra_udc_ep_setup(struct tegra_udc *udc)
{
	...
	static const struct usb_ep_caps _caps = USB_EP_CAPS(USB_EP_CAPS_TYPE_CONTROL, USB_EP_CAPS_DIR_ALL);
	int i;
	...
	udc->eps[0].ep.maxpacket_limit = USB_MAX_CTRL_PAYLOAD;
	udc->eps[0].ep.caps = _caps;
...
static int __init tegra_udc_probe(struct platform_device *pdev)
{
	...
	/* bleung added ppcs mapping from dt */
	...
	udc->ppcs_base = ioremap(res->start, resource_size(res));
	if (!udc->ppcs_base) {
		err = -ENOMEM;
		ERR("failed to map ppcs mem region\n");
		goto err_rel_mem_region;
	}
	...
	if (IS_ERR(udc->phy)) {
		dev_err(&pdev->dev, "failed to open USB phy, err: %ld\n",
				PTR_ERR(udc->phy));
		err = PTR_ERR(udc->phy);
		goto err_irq;
	}
	...
	/* External connector */
	udc->edev = devm_extcon_dev_allocate(&pdev->dev, tegra_udc_extcon_cable);
	if (!udc->edev) {
		dev_err(&pdev->dev, "failed to allocate memory for extcon\n");
		err = -ENOMEM;
		goto err_del_udc;
	}
	err = devm_extcon_dev_register(&pdev->dev, udc->edev);
	if (err) {
