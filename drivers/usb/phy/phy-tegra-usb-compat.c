...
#define USB_PLL_REG "avdd-usb-pll"
#else
#define USB_PLL_REG "avdd-pll-utmip"
#endif
...
/*FIXME: May be move to header later along with gizmo
 * restructuring*/
...
#if 0
	/* Benson's hack. Might need these ops in the future.. */
...
{
	void __iomem *ahb_gizmo = phy->gizmo_base;
	u32 val;
...
struct tegra_usb_phy *tegra_usb_phy_open(struct platform_device *pdev)
{
	...
	phy->gizmo_base = ioremap(res->start, resource_size(res));
	if (!phy->gizmo_base) {
		ERR("inst:[%d] Failed to remap I/O memory\n", phy->inst);
		err = -ENOMEM;
		goto fail_io;
	...
	tegra_init_ahb_gizmo_settings(phy);

	phy->vdd_reg = regulator_get(&pdev->dev, "avdd-usb");
	if (IS_ERR_OR_NULL(phy->vdd_reg)) {
		ERR("inst:[%d] couldn't get regulator avdd-usb: %ld\n",
